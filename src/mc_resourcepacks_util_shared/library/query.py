                        ...
                        with Path(sub_path, sub_file).open(mode="rb") as opened_file:
                            ...
                            if decoded is not None:
                                for line in decoded.splitlines(False):
                                    test_match_of_opened_file(directory, sub_file, line, query_builder)
                else:
                    query_when_or_not_regex(directory, Path(sub_path, sub_file), sub_file, query_builder)
                            ...
                            if query_builder.is_emissive_check:
                                if query_builder.patch in compressed_file:
                                    with zipped_file.open(compressed_file, mode="r", force_zip64=True) as opened_compressed_file:
                                        decoded: str | None = decode_bytes(opened_compressed_file.read())
                                        pprint(compressed_file, decoded is not None)
                                        if decoded is not None:
                                            pprint(len(decoded.splitlines(False)))
                                            for line in decoded.splitlines(False):
                                                pprint(line)
                                                test_match_of_opened_file(file, compressed_file, line, query_builder)
                            else:
                                query_when_or_not_regex(file, compressed_file, compressed_file, query_builder)
                        else:
                            query_when_or_not_regex(file, compressed_file, compressed_file, query_builder)
